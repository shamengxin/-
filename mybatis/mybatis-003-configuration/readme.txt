...
2.insert
    ...
    java程序中使用Map可以给SQL语句的占位符传值：
        ...
        注意：#{这里写什么？写map集合的key，如果key不存在，获取的null}
    ...
    java程序中使用POJO类给SQL语句的占位符传值：
        ...
        把sql语句写成这样：
            ...
            出现了什么问题呢？
                There is no getter for property named 'xyz' in 'class com.shamengxin.mybatis.pojo.Car'
                mybatis去找：Car类中的getXyz()方法去了，没找到，报错。

            怎么解决？
                可以在Car类中提供一个getXyz方法。这样问题就解决了。
            ...
            也就是说mybatis在底层给？传值的时候，先要获取值，怎么获取的？
                调用了pojo对象的get方法。例如：car.getCarNum(),car.getCarType(),car.getJBrand()
...
3.delete
    * 需求：根据id删除数据
    ...
    实现：
        ...
        注意：如果占位符只有一个，那么#{}的大括号里面可以随意。但是最好见名知意。
...
5.select（查一个，根据主键查询的话，返回的结果一定是一个。）
    * 需求：根据id查询。
    ...
    实现：
        <select id="selectById" resultType="com.shamengxin.mybatis.pojo.Car">
    ...
    需要特别注意的是：
        select标签中resultType属性，这个属性用来告诉mybatis，查询结果集封装成什么类型的java对象。需要告诉mybatis。
...
6.select（查所有）
    ...
    <select id="selectAll" resultType="com.shamengxin.mybatis.pojo.Car">
        select
            id,
            car_num as carNum,
            brand,
...
7. 在sql mapper.xml文件中有一个namespace，这个属性是用来指定命名空间的。用来防止id重复。
怎么用？
    在xml文件中：
        <mapper namespace="gdsf">
            <select id="selectAll" resultType="com.shamengxin.mybatis.pojo.Car">
                select
                    id,
                    car_num as carNum,
