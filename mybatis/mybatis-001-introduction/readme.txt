...
1. resource目录：
    ...
    直接放到resource目录下的资源，等同于放到了类的根路径下
...
2. 开发步骤
    ...
    * 第三步，编写mybatis核心配置文件：mybatis-config.xml
        注意：
            第一：这个文件名不是必须叫做mybatis-congfig.xml，可以用其他的名字。只是大家都采用这个名字。
            第二：这个文件存放的位置也不是固定的，可以随意，但一般情况下，会发放到类的跟路径下。

        mybatis-config.xml文件中的配置信息不理解没关系，先把连接数据库的信息修改一下即可。
        其他的别动。
    * 第四步：编写XxxMapper.xml文件
        在这个配置文件中编写sql语句
        这个文件名也不是固定的，放的位置也不是固定的，我们这里给它起个名字，叫做CarMapper.xml
        把它暂时放到类的根路径下。
...
5. 关于第一个程序的细节
    ...
    * InputStream is = new FileInputStream("d:\\mybatis-config.xml");
        采用这种方式也可以。
        缺点：可移植性太差，程序不够健壮，可能会一直到其他的操作系统当中。导致以上路径无效，还需要修改java代码中的路径，这样违背了OCP原则。
    * 已经验证了：
        mybatis核心配置文件的名字，不一定是：mybatis-config.xml。可以是其他路径。
...
6. 关于mybatis的事务管理机制。（深度剖析）
    ...
    * JDBC事务管理器：
        ...
        如果你编写的代码是下面的代码：
            ...
            如果在JDBC事务中，没有执行conn.setAutoCommit(false);那么autoCommit就是true。
            如果autoCommit是true，就表示没有开启事务，只要执行任意一条DML语句就提交一次。

    * MANAGED事务管理器：
        mybatis不再负责事务的管理了。事务的管理交给其他容器来负责。例如：spring。

        对于我们当前的单纯的只有mybatis的情况下，如果配置为：MANAGED
        那么事务这块是没人管的。没有人管理事务标识事务压根没有开启。
    ...
    * 重点：
        以后注意力，只要你的autoCommit是true，就表示没有开启事务。
